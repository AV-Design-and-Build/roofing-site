...
- Facebook public page still exposes stale data plus contact email `riseroofing9663@gmail.com`. Sent a NAP-correction request from `sales@avdesignandbuilds.com` to that address asking the account holder to update Facebook/Calendly-controlled profile data to the canonical phone, website, and estimate link. Gmail sent message ID: `19e1c4d13b612bec`. A same-thread follow-up asking for owner action or the correct owner/access path was sent as Gmail message ID `19e1cba03063d5c2`. The stale Facebook URL was also removed from the Google Business Profile social-profile field; after Google first showed the edit as pending, a refreshed public Google card showed only Instagram under Profiles and no Facebook link. Evidence screenshots: `/tmp/rise-google-social-facebook-removed-pending.png` and `/tmp/rise-google-public-card-facebook-social-removed.png`.
...
- Opened the Google Business Profile `Edit profile` -> `Contact` -> `Social profiles` editor.
- The profile had Instagram `https://www.instagram.com/rise_roofing_` and Facebook `https://www.facebook.com/profile.php?id=61580972774699&mibextid=wwXIfr&mibextid=wwXIfr`.
- Removed only the stale Facebook social profile from GBP and kept Instagram intact.
...
- Google Business Profile Support replied to the Calendly/provider-link support inquiry. Their response says third-party provider links come from providers who say they have authorized relationships with the business and that the provider should remove the business information from the data they send to Google. Gmail message ID: `19e1ceea60b63b32`.
- Replied in the same Google support thread with the exact Rise Roofing NAP, correct estimate URL, stale `calendly.com/riseroofing9663` link, and the fact that Rise Roofing does not authorize Calendly or control that Calendly account. Gmail sent message ID: `19e1cefc795b1112`.
- Tried Calendly's public support form. It accepted the request fields but did not submit from the browser session. Sent the same provider-feed removal request directly to `support@calendly.com`; Gmail sent message ID: `19e1cf108149c6b6`.
- Calendly opened support request `3672342`. Their chatbot said unauthorized Calendly links should be reported as impersonation/misuse unless a trademark takedown applies, and asked which route this is. Gmail message ID: `19e1cf1c31486f49`.
- Replied that this is primarily unauthorized use/impersonation, not a trademark takedown, and asked Calendly to route the ticket to a human agent. Gmail sent message ID: `19e1cf3bd460a5dd`.
- The Calendly abuse-report page's `Report abuse` button did not open a usable form from the browser session, so the Zendesk ticket reply is the current Calendly escalation path.
